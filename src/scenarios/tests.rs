...
fn test_full_scenario_workflow() {
    ...
    assert!(
        ...
        "Expected gross profit EV of 350K, got {}",
        ev_gross
    );
    ...
    assert!(
        ...
        "Expected net profit EV of 262.5K, got {}",
        ev_net
    );
}
...
fn test_probability_positive_calculation() {
    ...
    assert!(
        ...
        "Expected 0.60 prob positive, got {}",
        prob_pos
    );
...
fn test_ranges_calculation() {
    ...
    assert!(
        ...
        "Expected min 100K, got {}",
        min
    );
    assert!(
        (max - 600_000.0).abs() < 0.01,
        "Expected max 600K, got {}",
        max
    );
}
