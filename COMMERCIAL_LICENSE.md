# Commercial License

Forge is source-available under the [Elastic License 2.0](LICENSE).

## Free Use

The following uses are permitted without a commercial license:

| Use Case | Permitted |
|----------|-----------|
| Evaluation and testing | Yes |
| Internal development and staging | Yes |
| Personal and educational use | Yes |
| Non-production environments | Yes |

## Commercial License Required

A commercial license is required for:

- Production use in commercial environments
- Providing Forge as a hosted or managed service
- Enterprise deployments with support requirements

## What's Included

Commercial licenses include:

- Production deployment rights
- Priority support
- License indemnification
- Access to enterprise features (as available)

## Pricing

Pricing is determined based on your organization's size and use case.

**Contact us for licensing:**

Open a GitHub Issue with the `licensing` label:
- [github.com/royalbit/forge/issues](https://github.com/royalbit/forge/issues)

---

Copyright (c) 2025 RoyalBit Inc. All rights reserved.
